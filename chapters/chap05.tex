...
\subsection{Random Forest \& Decision Tree Classification}
...
\subsection{Gaussian Naive Bayes Classification}
...
\fig{img/titanic_all_models_boosting_experiments}{Comparing all model type performance, training data size boosted by 20\% with Wasserstein-GP generator}{fig:all_titanic}{1}
...
\subsection{Fitting the Generator and models on completely new data}

Two new datasets were selected as test sets for the architecture that was used:

\subsubsection{The Wine Quality Dataset}

Described by \cite{winequality}

\subsubsection{The Pima }